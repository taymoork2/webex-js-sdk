...
export default class LocusInfo {
  ...
  updateParticipants(participants, deltaParticpantFlag) {
    this.emitChanges(
      deltaParticpantFlag ? 'LOCUS_INFO_DELTA_PARTICIPANTS' : 'LOCUS_INFO_FULL_PARTICIPANTS',
      {
        participants,
  ...
  updateHostInfo(host) {
    if (host && !isEqual(this.host, host)) {
      ...
      if (parsedHosts.updates.isNewHost) {
        this.emitChanges('MEETING_HOST_newHost', {host});
      }
      this.host = host;
  ...
  updateMediaShares(mediaShares) {
    if (mediaShares && !isEqual(this.mediaShares, mediaShares)) {
      ...
      this.emitChanges('MEDIA_SHARES_CONTENT_UPDATE', {current: parsedMediaShares.current, previous: parsedMediaShares.previous});
      // if (parsedMediaShares.updates.isNewContentSharing) {
      //   this.emitChanges('MEDIA_SHARES_CONTENT_UPDATE', {updatedContentId: parsedMediaShares.current.contentId, removedContentId: parsedMediaShares.previous.contentId});
      // }
  ...
  updateSelf(self, participants) {
    if (self && !isEqual(this.self, self)) {
      ...
      this.emitChanges('LOCUS_SELF_UPDATE', self);
      this.self = self;

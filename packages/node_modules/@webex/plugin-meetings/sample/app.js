...
function getParticipantNameString(participant) {
  return `${participant.person.email},`;
}

function getParticipantValueString(member) {
  return `
  self: ${member.isSelf},
  host: ${member.isHost},
...
document.getElementById('join').addEventListener('click', () => {
  ...
  meeting.members.on('members:created', (members) => {
    let memberString = '';
    Object.keys(members).forEach((key) => {
      const member = members[key];
      memberString += `Member: ${getParticipantNameString(member.participant)} Values: ${getParticipantValueString(member)}\r\n`;
    });
    document.getElementById('participants-collection').innerHTML = `MEMBERS INFO:\r\n${memberString}`;
  });

  meeting.members.on('members:full', (members) => {
    let memberString = '';
    Object.keys(members).forEach((key) => {
      const member = members[key];
      memberString += `Member: ${getParticipantNameString(member.participant)}, Values: ${getParticipantValueString(member)}\r\n`;
